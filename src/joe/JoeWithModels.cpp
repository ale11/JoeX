...
void JoeWithModels::initialHook()
{
  ...
  Param *param;
  if (getParam(param, "INIT_PROFILE"))
  {
    // -------------------------------------------------------------------------
    ...
    fclose(fp);
    if (mpi_rank == 0)
      cout << "Read initial profile: " << fileIC << endl;
  ...
  else
  {
    ...
    // -------------------------------------------------------------------------
    double init_rho, init_p, init_u[3] = {0.0, 0.0, 0.0};

    Param *p;
    if (getParam(p, "INIT_RHO"))       init_rho = p->getDouble(1);
    else                               init_rho = rho_ref;

    if (getParam(p, "INIT_P"))         init_p = p->getDouble(1);
    else                               init_p = p_ref;

    if (getParam(p, "INIT_U"))
    {
      init_u[0] = p->getDouble(1);
      init_u[1] = p->getDouble(2);
      init_u[2] = p->getDouble(3);
    }
    ...
    if (!checkDataFlag(rhou))
      for (int icv=0; icv<ncv; icv++)
        for (int i=0; i<3; i++)
          rhou[icv][i] = init_rho*init_u[i];

    if (!checkDataFlag(rhoE))
      for (int icv=0; icv<ncv; icv++)
        rhoE[icv] = init_p/(gamma[icv]-1.0)+ 0.5*init_rho*vecDotVec3d(init_u, init_u);
  ...
  if ( fileBC != "none")
  {
    ...
    fclose(fp);
    if (mpi_rank == 0)
      cout << "Read boundary profile: " << fileBC << endl;
