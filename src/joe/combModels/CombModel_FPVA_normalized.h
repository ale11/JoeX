#ifndef COMBMODEL_FPVA_NORMALIZED_H_
#define COMBMODEL_FPVA_NORMALIZED_H_
 ...
 */
class RansCombFPVA_normalized: public RansCombBase {
public:
  // member variables

  ChemtableNormalized myChemTable;                  ///< Chemistry table based on flamelets.
...
//    rhoDkZ       = NULL;        registerScalar(rhoDkZ, "rhoDkZ", CV_DATA);
  ...
  virtual ~RansCombFPVA_normalized()
  {
    myChemTable.Unload();
...
public:
  ...
  virtual void calcStateVariables()
  {
    ...
    {
      Sz_out = myChemTable.ComputeNormalizedVariance(ZMean[icv], ZVar[icv]);
      C_out = myChemTable.ComputeNormalizedProg(ZMean[icv],Sz_out,CMean[icv]);

      Zv_norm[icv] = Sz_out;
      C_norm[icv]  = C_out;
      // Read and interpolate coefficients from chemistry table and save source term for progress variable
      myChemTable.LookupCoeff(RoM[icv], T0, E0, GAMMA0, AGAMMA, MU0, AMU, LOC0, ALOC, CmeanSource[icv], ZMean[icv], Sz_out, C_out);
      ...
      for (int i=0; i<OutputSpeciesName.size(); i++)
      {
        ...
        Species[icv] = myChemTable.Lookup(ZMean[icv], Sz_out, C_out, SpeciesName);
      }
...
//    updateCvData(rhoDkZ, REPLACE_DATA);
  ...
  {
    double R0, T;
    double  Sz_out = myChemTable.ComputeNormalizedVariance(ZM, ZV);
    double  C_out  = myChemTable.ComputeNormalizedProg(ZM,Sz_out,CM);
    myChemTable.LookupSelectedCoeff(R0, T0, E0, GAMMA0, AGAMMA, ZM, Sz_out, C_out);
    CMSource = myChemTable.Lookup(ZM, Sz_out, C_out, "SRC_PROG");
    if (AGAMMA == 0.0)
  ...
  virtual void calcThermoProp_T(double &p, double &h, double &R, double &gam, double &rho, double &T, double *Scal, int nScal)
  {
    ...
    myChemTable.LookupSelectedCoeff(R, T0, E0, GAMMA0, AGAMMA, zm, Sz_out, C_out);
    if (AGAMMA == 0.0)
  ...
  void ComputeProperties_T(double &h, double &RoM, double &gam, double &mu, double &lamOcp, double T, double zm, double zv, double cm)
  {
    ...
    myChemTable.LookupCoeff(RoM, T0, E0, GAMMA0, AGAMMA, MU0, AMU, LOC0, ALOC, dummy, zm, Sz_out, C_out);
    if (AGAMMA == 0.0)
...
#endif /* COMBMODEL_FPVA_NORMALIZED_H_ */
