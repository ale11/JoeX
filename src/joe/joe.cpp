...
public:
  ...
  virtual void writeAllValues()
  {
    ...
    {
      fprintf(fp, "%.8le\t%.8le\t%.12le\t%.12le\t%.12le\t%.12le\t",
...
/*
 * MyJoe with ASBMkom model
 */
class MyJoeASBMkom: public MyJoe, public RansTurbASBMkom
{
public:
  MyJoeASBMkom(char *name) : MyJoe(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0)
      cout << "MyJoeASBMkom()" << endl;
  }
...
public:
  double *duudx, *duvdy, *duvdx, *dvvdy;

public:
  NonPerChan(char *name) : MyJoe(name), UgpWithCvCompFlow(name)
  {
    if (mpi_rank == 0) cout << "NonPerChanSST()" << endl;

    registerScalar(kine, "kine", CV_DATA);
  }
  ...
  void initialHook()
  {
    duudx = new double[ncv_g];
    duvdy = new double[ncv_g];
    duvdx = new double[ncv_g];
    dvvdy = new double[ncv_g];
  ...
  virtual void finalHook()
  {
    ...
    if (duudx != NULL) {delete [] duudx;    duudx = NULL;}
    if (duvdy != NULL) {delete [] duvdy;    duvdy = NULL;}
    if (duvdx != NULL) {delete [] duvdx;    duvdx = NULL;}
    if (dvvdy != NULL) {delete [] dvvdy;    dvvdy = NULL;}
  }
...
//    for (int icv = 0; icv < ncv; icv++)
//    {
//      rhs_rhou[icv][0] += cv_volume[icv]*(duudx[icv] + duvdy[icv]);
//      rhs_rhou[icv][1] += cv_volume[icv]*(duvdx[icv] + dvvdy[icv]);
...
//  }
  ...
  void readReynoldsStress()
  {
    ...
    for (int icv=0; icv<ncv; icv++)
    {
      ...
      rey1 = re11 + fx*(re12 - re11);
      rey2 = re21 + fx*(re22 - re21);
      press[icv] = rey1 + fy*(rey2 - rey1);

      re11 = reStress[posx-1][posy-1][4];
      re12 = reStress[posx][posy-1][4];
      re21 = reStress[posx-1][posy][4];
      re22 = reStress[posx][posy][4];
      rey1 = re11 + fx*(re12 - re11);
      rey2 = re21 + fx*(re22 - re21);
      rij_diag[icv][0] = rey1 + fy*(rey2 - rey1);

      re11 = reStress[posx-1][posy-1][5];
      re12 = reStress[posx][posy-1][5];
      re21 = reStress[posx-1][posy][5];
      re22 = reStress[posx][posy][5];
      rey1 = re11 + fx*(re12 - re11);
      rey2 = re21 + fx*(re22 - re21);
      rij_diag[icv][1] = rey1 + fy*(rey2 - rey1);

      re11 = reStress[posx-1][posy-1][6];
      re12 = reStress[posx][posy-1][6];
      re21 = reStress[posx-1][posy][6];
      re22 = reStress[posx][posy][6];
      rey1 = re11 + fx*(re12 - re11);
      rey2 = re21 + fx*(re22 - re21);
      rij_diag[icv][2] = rey1 + fy*(rey2 - rey1);

      re11 = reStress[posx-1][posy-1][7];
      re12 = reStress[posx][posy-1][7];
      re21 = reStress[posx-1][posy][7];
      re22 = reStress[posx][posy][7];
      rey1 = re11 + fx*(re12 - re11);
      rey2 = re21 + fx*(re22 - re21);
    ...
    for (int icv = 0; icv < ncv; icv++)
    {
      double rho_icv = rho[icv] ;
      kine[icv] = 0.5*(-rij_diag[icv][0]/rho_icv
                       -rij_diag[icv][1]/rho_icv
                       -rij_diag[icv][2]/rho_icv);
    }
  ...
  void computeRestresGradients()
  {
    ...
    for (int ifa = nfa_b; ifa < nfa; ++ifa)
    {
      ...
      duudx[icv0] += rij_diag_fa[ifa][0]*fa_normal[ifa][0];
      duudx[icv1] -= rij_diag_fa[ifa][0]*fa_normal[ifa][0];

      duvdy[icv0] += rij_offdiag_fa[ifa][0]*fa_normal[ifa][1];
      duvdy[icv1] -= rij_offdiag_fa[ifa][0]*fa_normal[ifa][1];

      duvdx[icv0] += rij_offdiag_fa[ifa][0]*fa_normal[ifa][0];
      duvdx[icv1] -= rij_offdiag_fa[ifa][0]*fa_normal[ifa][0];

      dvvdy[icv0] += rij_diag_fa[ifa][1]*fa_normal[ifa][1];
      dvvdy[icv1] -= rij_diag_fa[ifa][1]*fa_normal[ifa][1];
    }

    for (int ifa = 0; ifa < nfa_b; ++ifa)
    {
      const int icv = cvofa[ifa][0];
      duudx[icv] += rij_diag_fa[ifa][0]*fa_normal[ifa][0];
      duvdy[icv] += rij_offdiag_fa[ifa][0]*fa_normal[ifa][1];
      duvdx[icv] += rij_offdiag_fa[ifa][0]*fa_normal[ifa][0];
      dvvdy[icv] += rij_diag_fa[ifa][1]*fa_normal[ifa][1];
    }

    for (int icv = 0; icv < ncv; ++icv)
    {
      duudx[icv] /= cv_volume[icv];
      duvdy[icv] /= cv_volume[icv];
      duvdx[icv] /= cv_volume[icv];
      dvvdy[icv] /= cv_volume[icv];
    }
  }
