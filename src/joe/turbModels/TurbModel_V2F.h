...
public:
  RansTurbV2F()
  {
    ...
    eq->phiZero = 1.0e-8;
    eq->phiZeroRel = getDoubleParam("ZERO_REL_eps", "1.0e-2");
    eq->phiMaxiter = 500;
    eq->lowerBound = 1.0e-6;
    eq->upperBound = 1.0e10;
    ...
    eq->phiZero = 1.0e-9;
    eq->phiZeroRel = getDoubleParam("ZERO_REL_v2", "1.0e-2");
    eq->phiMaxiter = 500;
    ...
    eq->phiZero = 1.0e-8;
    eq->phiZeroRel = getDoubleParam("ZERO_REL_f", "1.0e-2");
    eq->phiMaxiter = 1000;
    eq->lowerBound = 0.0;
    eq->upperBound = 1.0e10;
...
public:
  ...
  virtual void sourceHookScalarRansTurb_new(double *rhs, double *A, const string &name, int flagImplicit)
  {
    ...
    {
      int noc00;
      double nuLamCV, TS, Pv, Pv_limit, src, dsrcdphi;
  ...
  virtual void sourceHookRansTurbCoupled(double **rhs, double ***A, int nScal, int flagImplicit)
  {
    ...
    double nuLamCV, TS, LS, LS2, ce1, dsrcdphi, Pv, Pv_limit;
    double kine_src, eps_src, v2_src, f_src;
